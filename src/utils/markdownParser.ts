...
export const parsePracticeLabMarkdown = (content: string): ParsedLab => {
    ...
    sections.forEach((section) => {
        ...
        // Diagnostic
        if (lowerFirst.includes('diagnóstico') || firstLine.includes('🎯')) {
            diagnostic = section.replace(/^[^\n]*\n/, '').trim();
        } 
        // Methodology
        else if (lowerFirst.includes('metodología') || firstLine.includes('📚')) {
            methodology = section.replace(/^[^\n]*\n/, '').trim();
        }
        // Exercise
        else if (firstLine.includes('🔨') || lowerFirst.includes('ejercicio')) {
            const levelMatch = firstLine.match(/Ejercicio\s*(\d+)/i) || section.match(/(\d+)/);
            const level = levelMatch ? parseInt(levelMatch[1], 10) : 0;

            if (level > 0 && level <= 5) {
                // Isolated Rubric and Answer extraction
                const rubricMatch = section.match(/(?:\s*###|>\s*|>\s*📋\s*)(?:📋|Rúbrica|auto-evaluación)[\s\S]*?(?=(?:\s*###|>\s*|>\s*💡\s*)(?:💡|Respuesta|Respuestas)|$)/i);
                const answerMatch = section.match(/(?:\s*###|>\s*|>\s*💡\s*)(?:💡|Respuesta|Respuestas)[\s\S]*$/i);

                // Clean Description: Excludes Title line, then stops before rubric/answer triggers
                const description = section
                    .replace(/^[^\n]*\n/, '') // removes title
                    .split(/(?:\s*###|>\s*)(?:📋|💡|Rúbrica|Respuesta)/i)[0]
                    .trim();
